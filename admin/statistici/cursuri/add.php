...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$date || !is_array($participants) || empty($participants)) {
        $error = !$date && $dateRaw
            ? 'Nu am putut interpreta data "' . $dateRaw . '". Incearca: 6 martie, 6.03 sau 6.03.2026.'
            : 'Completeaza toate campurile si incarca fisierul XLSX.';
    } else {
        ...
        header("Location: /admin/statistici/cursuri/view.php?id={$courseId}");
        exit;
    }
}
?>
...
<?php include __DIR__ . '/../layout_nav.php'; ?>
  ...
  <div class="add-wrap">
    ...
    <form method="post" id="addForm">
      ...
      <div class="card">
        <div class="form-group" style="margin-bottom:0">
          <label>Lista participanti (XLSX)</label>
          <div class="drop-zone" id="dropZone" style="margin-top:8px">
            <input type="file" id="fileInput" accept=".xlsx,.xls,.csv">
            <div class="drop-icon">📊</div>
            <div class="drop-title">Trage fisierul sau apasa pentru selectare</div>
      ...
      <div class="col-picker" id="colPicker">
        ...
        <button type="button" class="btn btn-ghost" id="btnApplyCol" style="margin-top:10px;width:100%;justify-content:center">Aplica</button>
      </div>
      ...
      <div id="submitWrap" style="display:none;margin-top:20px">
        <button type="submit" class="btn btn-green" style="width:100%;justify-content:center;padding:12px" id="btnSave">Salveaza cursul</button>
      </div>
    </form>
  </div>
...
function handleFile(file) {
    ...
    reader.onload = e => {
        try {
            ...
            if (!parsedRows.length) { alert('Fisierul pare gol.'); return; }
        ...
        } catch (err) { alert('Nu am putut citi fisierul.'); }
    };
...
function applyCol(col) {
    ...
    if (!names.length) { alert('Coloana selectata pare goala.'); return; }
    ...
    sorted.forEach(({ n, o }) => {
        ...
        li.innerHTML = `<span class="preview-bullet"></span> <strong>${o}</strong>&nbsp;${o===1?'comanda':'comenzi'} × <strong>${n} ${n===1?'bilet':'bilete'}</strong>`;
        list.appendChild(li);
    });
...
document.getElementById('courseDate').addEventListener('input', function() {
    ...
    if (parsed) {
        ...
        el.textContent = '✕ data nerecunoscuta';
    }
});
</script>
    </main>
